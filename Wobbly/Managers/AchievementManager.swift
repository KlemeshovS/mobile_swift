...
class NewAchievementManager {
    ...
    func recalculateAllAchievements(daysData: [String: DrinkLevel]) -> [Achievement] {
        ...
        for index in currentAchievements.indices {
            ...
            if !shouldUnlock && achievement.isUnlocked {
                // Ачивки с ручной разблокировкой, milestone-вехи и счётчики пари не
                // сбрасываем автоматически: отзыв — потому что он вручную подтверждается,
                // milestone — потому что однажды достигнутая веха (высота/глубина)
                // должна оставаться в истории достижений, даже если текущий прогресс упал
                // ниже порога, betOutcomeCount — потому что это кумулятивный исторический
                // счётчик (сыграно/выиграно/проиграно/отклонено), который не должен
                // "просаживаться" сам по себе, а на практике мог false-negative на этой
                // самой проверке из-за того, что BetsManager.shared.bets — синхронный кэш,
                // который на холодном старте ещё пуст (сервер не успел ответить).
                switch achievement.type {
                case .leftReview, .milestone, .betOutcomeCount:
                    continue
                default:
                    break
                }
